...
const MenteActivaLogo = ({ className = "", showDetails = true, align = "center", imgHeight = "h-full", smallHeight = "48px", style = {}, forceDark = false }) => {
    const { theme } = useAuth();
            ...
            {showDetails && (
                <div className="flex flex-col items-center mt-2 pointer-events-none">
                    <div className="flex gap-2 text-[32px] font-black tracking-tighter leading-none" style={{ fontFamily: 'system-ui, -apple-system, sans-serif' }}>
                        <span className={isDark ? "text-white" : "text-slate-800"}>MENTE</span>
                        <span className="text-transparent bg-clip-text bg-gradient-to-br from-[#d97706] to-[#fbbf24]">ACTIVA</span>
                    </div>
                    <div className={`text-[11px] font-bold tracking-[0.18em] mt-1 uppercase text-center ${isDark ? "text-slate-300" : "text-slate-700"}`}>
