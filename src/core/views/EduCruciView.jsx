...
const EduCruciView = () => {
    ...
    const [modo, setModo] = useState('crucigrama');
...
- PROHIBICIÓN DE GENERACIÓN DE APLICACIONES/INTERACTIVOS: ESTÁ ESTRICTAMENTE PROHIBIDO GENERAR CÓDIGO DE PROGRAMACIÓN, APLICACIONES INTERACTIVAS, COMPONENTES DINÁMICOS, ARTEFACTOS DE CÓDIGO (COMO REACT, HTML/JS, PYTHON) O INICIAR MÓDULOS DE EJECUCIÓN/COMPILACIÓN EN EL CHAT. TU RESPUESTA DEBE SER EXCLUSIVAMENTE TEXTO PLANO ESTATUADO SEGÚN EL FORMATO SOLICITADO. NO CREES NINGÚN JUEGO DE CRUCIGRAMA INTERACTIVO NI CÓDIGO DE APLICACIÓN.`;
                        ...
                        <div className="space-y-2 pt-2">
                            ...
                            <div className="grid grid-cols-1 gap-2">
                                <label className={`flex items-center gap-3 p-2 rounded-xl border cursor-pointer transition-all ${enfoque === 'personajes' ? 'bg-blue-600/20 border-blue-500' : 'bg-[#0f172a] border-slate-700 hover:border-slate-500'}`}>
                                    <input type="radio" name="enfoque_cruci" value="personajes" checked={enfoque === 'personajes'} onChange={() => setEnfoque('personajes')} className="hidden" />
                                ...
                                <label className={`flex items-center gap-3 p-2 rounded-xl border cursor-pointer transition-all ${enfoque === 'escolar' ? 'bg-emerald-600/20 border-emerald-500' : 'bg-[#0f172a] border-slate-700 hover:border-slate-500'}`}>
                                    <input type="radio" name="enfoque_cruci" value="escolar" checked={enfoque === 'escolar'} onChange={() => setEnfoque('escolar')} className="hidden" />
                                    <div className="text-xl">📚</div>
                                    <div className="flex flex-col">
                                ...
                                <label className={`flex items-center gap-3 p-2 rounded-xl border cursor-pointer transition-all ${enfoque === 'valores' ? 'bg-rose-600/20 border-rose-500' : 'bg-[#0f172a] border-slate-700 hover:border-slate-500'}`}>
                                    <input type="radio" name="enfoque_cruci" value="valores" checked={enfoque === 'valores'} onChange={() => setEnfoque('valores')} className="hidden" />
                ...
                <aside className="w-[320px] flex-shrink-0 bg-[#1e293b] border-l border-slate-800 flex flex-col z-20 shadow-[-10px_0_30px_rgba(0,0,0,0.3)] overflow-y-auto print:hidden">
                    <div className="p-6 pt-10 space-y-8">
                        ...
                        <div className="relative overflow-hidden bg-[#0f172a] border border-slate-700/50 rounded-[2rem] p-6 shadow-xl flex flex-col items-center text-center mx-2">
                            <div className="absolute top-0 right-0 w-32 h-32 bg-orange-500 opacity-[0.05] rounded-full blur-3xl -mr-10 -mt-10"></div>
                            <div className="relative z-10 flex flex-col items-center">
                                <div className="w-24 h-24 rounded-3xl flex items-center justify-center mb-5 shadow-inner" style={{ backgroundColor: '#f9731615' }}>
                                    <img src={crucigramaIcon} alt="Crucigramas" className="w-20 h-20 object-contain drop-shadow-lg" />
                                </div>
                        ...
                        <div>
                            ...
                            <div className="space-y-3">
                                <button onClick={() => procesar('crucigrama')} className="w-full py-4 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl font-black uppercase text-[10px] flex items-center justify-center gap-2 shadow-lg active:scale-95 transition-all"><FileText size={14} /> Generar Crucigrama</button>
                                <button 
                                    id="BTN_PRINT_NATIVE"
